...
class LogoutOtherBrowserSessionsForm extends Component
{
    ...
    public function logoutOtherBrowserSessions(): void
    {
        ...
        if (! Hash::check($this->password, Auth::user()->password)) {
            throw ValidationException::withMessages([
                'password' => [__('political-flow::default.errors.invalid_password')],
            ]);
        }
    ...
    {
        return view('political-flow::profile.logout-other-browser-sessions-form');
    }

    public function browserSessionsTerminated(): void
    {
        Notification::make()
            ->title(__('political-flow::default.notifications.browser_sessions_terminated.title'))
            ->success()
            ->body(__('political-flow::default.notifications.browser_sessions_terminated.body'))
            ->send();
    }
}
